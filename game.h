...
public:
	...
	std::map<int, int>::const_iterator begin() const { return msp->begin(); }
	std::map<int, int>::const_iterator end() const { return msp->end(); }
private:
	Smart_Ptr< std::map<int, int> > msp;
};
...
public:
	...
	// The returned std::vector< std::vector<int> > records in its jth element  
	// the Bricks which were added to column j of the board (for 0 <= j < M).
...
// Take a const reference to a std::vector<Brick> and return a std::vector of 
// the ranks
...
// Return the same std::vector as abs_sort_Brick_indices, except with the 1st  
// and 2nd elements swapped, the third and fourth elements swapped etc.
...
std::map<int, int> str_to_map(const std::string&);
